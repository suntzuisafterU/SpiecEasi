  ...
  if (length(est$refit) > 0) {
    if (names(est$refit) %in% "stars") {
      return(switch(getter,
        ...
        icov = {
          if (est$est$method %in% c("slr", "glasso"))
            est$est$icov[[getOptInd(est)]]
          else
          stop("Run spiec-easi with method=\"glasso\" or method=\"slr\"")
        },
        cov = {
        ...
        beta = {
          if (est$est$method %in% c("mb", "ising", "poisson"))
            est$est$beta[[getOptInd(est)]]
          else
            stop("Run spiec-easi with method=\"mb\" \"ising\" or \"poisson\"")
        }
      ))
    } else
      stop("Run spiec.easi with sel.criterion=\"stars\" or \"bstars\"")
  } else
    stop("Run spiec.easi with pulsar.select=TRUE or \"batch\"")
}
